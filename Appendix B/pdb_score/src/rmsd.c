                              ...
                              double* residual)
{
   ...
   */
   ...
   /* NOTE: the comments on the original source of this code found a bug:
   
   (from http://boscoh.com/protein/rmsd-root-mean-square-deviation):
   
	left_eigenvec[i][j] = dot(&right_eigenvec[i][0], &Rt[j][0]);
	should be read as
	left_eigenvec[i][j] = dot(&right_eigenvec[i][0], &R[j][0]);
...
{ 
  ...
  *rmsd = sqrt( (double) residual*2 / ((double) n_list) ); 
}
