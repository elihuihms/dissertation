	...
	def __init__(self, distanceMatrix):
		...
		for i in range(self.size):
			for j in range(i + 1, self.size):
				...
				dList.append((d, k))
				self.matrix[k] = d
		self.distances = SortedList(dList)
		step = 0
		self.avSp = []
	...
	def _reduce(self, step):
		...
		while True:
			...
			for c in self.clusters:
				...
				dList.append((d, k))
				self.matrix[k] = d
			self.distances.merge(SortedList(dList))
...
class SortedList:
	...
	def merge(self, other):
		...
		if not self.values:
			...
			return
		nList = []
		mySize = len(self.values)
		otherSize = len(other.values)
		myIndex = 0
		myKey = self.values[0][0]
		otherIndex = 0
		otherKey = other.values[0][0]
		while True:
			if myKey <= otherKey:
				nList.append(self.values[myIndex])
				myIndex += 1
				if myIndex >= mySize:
					break
				myKey = self.values[myIndex][0]
			else:
				nList.append(other.values[otherIndex])
				otherIndex += 1
				if otherIndex >= otherSize:
					break
				otherKey = other.values[otherIndex][0]
